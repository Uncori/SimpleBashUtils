...
int flag_f(char *stroka, struct s_flag *flags)
{
    ...
    if (file == NULL) {
        ...
        while (getline(&line, &len, file) != -1) {
            ...
            snprintf(flags->template[flags->count_template++], str_len, "%s",
                     line);
        }
        free(line);
        fclose(file);
            ...
            for (size_t j = 1; j < strlen(argv[i]) && non_SOS == 0; ++j) {
                if (argv[i][j] == 'e') {
                    ...
                    if (j == strlen(argv[i] + 1)) {
                        i++;
                        snprintf(flags->template[flags->count_template++], LEN,
                                 "%s", argv[i]);
                        non_SOS++;
...
{
    struct s_flag flags;
    flags.e = 0, flags.f = 0, flags.i = 0, flags.v = 0, flags.c = 0,
    flags.l = 0, flags.n = 0, flags.h = 0, flags.s = 0, flags.count_flag = 0,
